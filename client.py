...
OPE_KEY = b'CLESECRETE' 
cipher_ope = OPE(OPE_KEY)

# Clés Paillier save dans un fichier paillier_keys.json
KEY_FILE = 'paillier_keys.json'

def load_or_generate_paillier_keys():
    if os.path.exists(KEY_FILE):
        with open(KEY_FILE, 'r') as f:
            keys = json.load(f)
            pub_key = paillier.PaillierPublicKey(n=int(keys['p_n']))
            priv_key = paillier.PaillierPrivateKey(pub_key, p=int(keys['s_p']), q=int(keys['s_q']))
            return pub_key, priv_key
    else:
        print("Génération d'une nouvelle paire de clés")
        pub_key, priv_key = paillier.generate_paillier_keypair()
        with open(KEY_FILE, 'w') as f:
            # p_n = n du public key, s_p et s_q = les facteurs premiers de la clé privée
            json.dump({'p_n': str(pub_key.n), 's_p': str(priv_key.p), 's_q': str(priv_key.q)}, f)
        return pub_key, priv_key
...
def main():
    ...
    while True:
        ...
        if choix == '1':
            ...
            id1 = input("ID de l'employé 1 : ")
            id2 = input("ID de l'employé 2 : ")
